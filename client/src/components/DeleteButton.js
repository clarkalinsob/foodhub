...
function DeleteButton({ mealId, foodId, commentId, mealDateId, typename, callback }) {
    ...
    update(proxy) {
      ...
      if (!commentId && !mealDateId) {
        if (foodId) {
          ...
          data.getFoods = data.getFoods.filter(food => food.id !== foodId)
          proxy.writeQuery({ query: FETCH_FOODS_QUERY, data })
        } else {
          const data = proxy.readQuery({
            query: FETCH_MEALS_QUERY
          })
          data.getMeals = data.getMeals.filter(meal => meal.id !== mealId)
  ...
  return (
    <>
      <Button as='div' color='google plus' floated='right' onClick={() => setModalOpen(true)}>
