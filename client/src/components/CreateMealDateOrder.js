...
function CreateMealDateOrder({ user, mealId, mealDate: { id, date, menu, orders } }) {
  ...
  const [createMealDateOrder, { error }] = useMutation(CREATE_MEALDATE_ORDER_MUTATION, {
    variables: {
      mealId,
      mealDateId: id,
      date,
        ...
        <Message key={thisOrder.id} positive size='small'>
          <Message.Header>
            <div style={{ float: 'right' }}>
              <Icon link name='edit' onClick={() => setEdit(true)} />{' '}
            </div>
            <Icon name='food' /> {thisOrder.foodName} <br /> <Icon name='clock' />{' '}
            {thisOrder.mealTime} <br />{' '}
          </Message.Header>
        </Message>
      ) : (
        <>
            ...
            <Message warning size='small'>
              <Message.Header>
                ...
                <span style={{ float: 'right' }}>
                  <Icon link name='close' onClick={() => setEdit(false)} />{' '}
                </span>
              </Message.Header>
              <Message.Content>Please select a food and a meal time.</Message.Content>
            </Message>
          ) : (
            <Message
              warning
